...
**CRITICAL: Job files MUST be created in the project-relative path `.claude/olaclaw/jobs/`, NOT in `~/.claude/olaclaw/jobs/`.** The daemon only watches the project directory. Using the home directory path will silently fail — the job will never fire.
...
1. List all `.md` files in `.claude/olaclaw/jobs/`.
...
3. Create the job file at `.claude/olaclaw/jobs/<name>.md` with this exact format:
   ```markdown
   ---
...
1. Read `.claude/olaclaw/jobs/<job-name>.md`. If it doesn't exist, list available jobs and ask the user which one to edit.
...
3. If confirmed, delete `.claude/olaclaw/jobs/<job-name>.md`.
...
1. Read `.claude/olaclaw/jobs/<job-name>.md`. If it doesn't exist, list available jobs.
...
Jobs live in `.claude/olaclaw/jobs/` as markdown files:
